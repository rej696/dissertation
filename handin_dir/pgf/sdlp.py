...
class TcSpaceDataLinkProtocolHeader:
    def __init__(
        ...
        if scid > SCID_MAX or scid < 0:
            raise ValueError(
                "Invalid scid value, not in range" f" 0 < {vcid} <= {SCID_MAX}"
            )
        if vcid > VCID_MAX or vcid < 0:
            raise ValueError(
                "Invalid vcid value, not in range" f" 0 < {vcid} <= {VCID_MAX}"
            )
    ...
    def pack(self) -> bytearray:
        ...
        header = bytearray()
        first_word = (
            ((self.ccsds_version << VERSION_OFFSET) & VERSION_MASK)
            | ((self.bypass_flag << BYPASS_FLAG_OFFSET) & BYPASS_FLAG_MASK)
            | (
                (self.control_command_flag << CONTROL_COMMAND_FLAG_OFFSET)
                & CONTROL_COMMAND_FLAG_MASK
            )
            | (self.scid & SCID_MASK)
        )
    ...
    def unpack(cls, data: bytes) -> Self:
        ...
        control_command_flag = (
            first_octet & CONTROL_COMMAND_FLAG_MASK
        ) >> CONTROL_COMMAND_FLAG_OFFSET
